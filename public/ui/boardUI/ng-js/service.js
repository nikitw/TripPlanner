...
var BASEURL = "http://tripplanner-cs6240nwaghela.rhcloud.com/";
...
services.factory('userService', function($http, $users, sessService) {
    ...
    us.updateUser = function (request) {
        ...
        $http.put(BASEURL+"users/user?apiKey="+user.apiKey, request.body)
            .success(function (response) {
                if(!response.err) {
    ...
    us.getUserById = function (request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired!', 701));
            return;
        }
        $http.jsonp(BASEURL+"users/user/"+request.body._id+"?callback=JSON_CALLBACK&apiKey="+user.apiKey)
            .success(function (response) {
                if(!response.err) {
                    request.success(response.body);
                } else {
...
services.factory('tripService', function ($http, $users, $trips, sessService, flimageService) {
    var ts = {};
    var user;
    ...
    ts.createTrip = function (request) {
        ...
        flimageService.getImageUrl(request.body, function(source) {
            console.log(source);
    ...
    ts.getPostedTrips = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        }
        $http.jsonp(BASEURL+'trips/posts?callback=JSON_CALLBACK&apiKey='+user.apiKey)
            .success(function (response) {
                if(!response.err) {
                    request.success($trips.getPostedTrips(response.body));
                } else {
                    request.error(response.err);
                }
            });
    ...
    ts.joinTrip = function(request) {
        ...
        $http.post(BASEURL+'trips/join?apiKey='+user.apiKey, request.body)
            .success(function (response) {
                if(!response.err) {
                    request.success($trips.getTrip(response.body));
                } else {
    ...
    ts.unjoinTrip = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        ...
        $http.post(BASEURL+'trips/unjoin?apiKey='+user.apiKey, request.body)
            .success(function (response) {
                if(!response.err) {
                    request.success($trips.getTrip(response.body));
                } else {
...
services.factory('peopleService', function (confService, sessService, $http, $people) {
    var ps = {};

    var user;

    ps.getTripsters = function (request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        ...
        $http.jsonp(BASEURL+'users?callback=JSON_CALLBACK&apiKey='+user.apiKey)
            .success(function (response) {
                if(!response.err) {
                    request.success($people.getTripsters(response.body));
                } else {
                    request.error(response.err);
                }
            });
    ...
    ps.getFriends = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        ...
        $http.jsonp(BASEURL+'users/friends?callback=JSON_CALLBACK&apiKey='+user.apiKey)
            .success(function (response) {
                if(!response.err) {
                    request.success($people.getFriends(response.body));
                } else {
                    request.error(response.err);
                }
            });
    ...
    ps.acceptRequest = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        }
        $http.post(BASEURL+'users/friends/accept?apiKey='+user.apiKey, request.body)
            .success(function (response) {
                if(!response.err) {
                    request.success();
                } else {
    ...
    ps.addFriend = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        }
        $http.post(BASEURL+'users/friends/request?apiKey='+user.apiKey, request.body)
            .success(function (response) {
                if(!response.err) {
                    request.success();
                } else {
    ...
    ps.removeFriend = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        }
        $http.delete(BASEURL+'users/friends/'+request.body._id+'?apiKey='+user.apiKey)
            .success(function (response) {
                if(!response.err) {
                    request.success();
                } else {
    ...
    ps.removeRequest = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        }
        $http.delete(BASEURL+'users/friends/request/'+request.body._id+'?apiKey='+user.apiKey)
            .success(function (response) {
                if(!response.err) {
                    request.success();
                } else {
    ...
    ps.getFriendRequests = function(request) {
        user = sessService.getSession('user');
        if(!user) {
            request.error(new Error('Session Expired', 701));
            return;
        }
        $http.jsonp(BASEURL+'users/friends/request?callback=JSON_CALLBACK&apiKey='+user.apiKey)
            .success(function (response) {
                if(!response.err) {
                    request.success($people.getRequests(response.body));
                } else {
                    request.error(response.err);
                }
            });
...
services.factory('pingerService', function(tripService, peopleService) {
    var pinger = {};

    pinger.reloadPeerData = function(callback) {
        peopleService.getFriends(new Request({}, function() {
            peopleService.getFriendRequests(new Request({}, function() {
                tripService.getTrips(new Request({}, function() {
                    tripService.getPostedTrips(new Request({}, function() {
                        if(callback)
                            callback();
                    }, function() {}));
                }, function() {}));
...
services.factory('flimageService', function($http) {
    var fls = {};

    function getLocation(city, callback) {
        var address = city.replace(/\s+/, '+');
    ...
    fls.getImageUrl = function(trip, callback){
        getLocation(trip.dest, function(loc) {
            if(loc) {
                $http.get("https://api.flickr.com/services/rest/?method=flickr.photos.search&text=skyline&lat="+loc.lat+"&lon="+loc.lng+"&per_page=3&api_key=a679104c092226d7150e1b5a399d083a&format=json&nojsoncallback=1")
                    .success(function(data) {
                        if(data.photos.photo.length != 0) {
                            $http.get("https://api.flickr.com/services/rest/?method=flickr.photos.getSizes&photo_id=" + data.photos.photo[0].id + "&format=json&api_key=a679104c092226d7150e1b5a399d083a&nojsoncallback=1")
                                .success(function (photos) {
                                    if (photos.sizes.size.length != 0)
                                        trip.img = photos.sizes.size[photos.sizes.size.length - 1].source;
                                    if (callback) {
                                        callback(trip.img);
                                    }
                                });
...
function getDatesString(trips) {
    var dates = [];
    for(var d in trips) {
        getStringDates(new Date(trips[d].start), new Date(trips[d].end), dates);
    }

    return dates;
}

function getStringDates(startDate, stopDate, dateArray) {
